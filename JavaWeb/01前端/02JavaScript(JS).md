...
```
### JS程序优化
内部JS复用性差，所以使用外部的形式，然后在script标签里用src引入
js里面的重复操作可以写在工具类.js里面,工具类里面的方法用export修饰使其暴露，其他js使用的时候用import引入
格式：
```
import { 方法名 } from "./util.js";
```
当html想要使用带有import，export等模块化语法的js时，要在script里面写上type="module"表示使用的是模块化的js
js模块化是高级语法
