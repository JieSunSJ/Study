...
```
### 题目 1（入门级：基础统计）
**需求**：查询每个职位（`job`）的员工数量，并按人数**从高到低**排序。
> 💡 提示：注意 `job` 字段有空值（`NULL`），`COUNT` 统计时是否会计数 `NULL`？
```
SELECT job, COUNT(*) AS 人数
FROM emp
GROUP BY job
ORDER BY 人数 DESC;  -- 或者 ORDER BY COUNT(*) DESC
```
### 题目 2（进阶级：聚合计算）
**需求**：查询每个职位（`job`）的**最高薪资**、**最低薪资**和**平均薪资**，要求平均薪资保留 2 位小数。
> 💡 提示：使用 `ROUND(avg(salary), 2)`；注意 `salary` 也有空值，聚合函数（`AVG`/`MAX`）会自动忽略 `NULL`。
```
SELECT 
    job,
    MAX(salary) AS 最高薪资,
...
```
###  题目 3（进阶级：日期函数 + 分组）
**需求**：查询**每一年**入职的员工人数，显示格式为“年份”和“人数”，并按年份升序排列。
> 💡 提示：使用 `YEAR(entry_date)` 来提取年份。
```
select YEAR(entry_date) AS 年份,count(*) AS 人数
from emp
GROUP BY 年份
ORDER BY 年份 asc;
```
### 题目 4（重点难点：HAVING 筛选分组后结果）
**需求**：查询**平均薪资大于 8000** 的职位，显示职位和平均薪资，并按平均薪资降序排列。
> 💡 思考：这里不能用 `WHERE salary > 8000`，应该用什么关键字来过滤分组后的数据？
```
select job,avg(salary) AS 平均薪资
from emp
GROUP BY job
HAVING 平均薪资>8000
ORDER BY 平均薪资 DESC;
```
### 附加题（挑战级：多字段分组）
**需求**：查询每个职位（`job`）、每种性别（`gender`）对应的**平均薪资**和**人数**，显示结果按职位升序、性别升序排列。
> 💡 提示：`GROUP BY` 后面可以跟多个字段（`GROUP BY job, gender`）。
```
SELECT 
    job AS 工作,
    gender AS 性别,
